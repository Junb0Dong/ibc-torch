...
from ibc.policy.simulation_pushing_state_policy import SimulationPushingStatePolicy   # TODO：换成能量模型的训练和推理
...
class SimulationPushingStateWorkspace(BaseWorkspace): # 从基类BaseWorkspace继承
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: SimulationPushingStatePolicy = hydra.utils.instantiate(cfg.policy)    # create instance of policy class
            ...
            for local_epoch_idx in range(cfg.training.num_epochs): # 训练轮数
                ...
                if (self.epoch % cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        obs_dict = batch['obs']
                        gt_action = batch['actions'] # ground_truth action
...
def main(cfg):
    workspace = SimulationPushingStateWorkspace(cfg)
    workspace.run()
